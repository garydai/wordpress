...
function wp_insert_attachment($object, $file = false, $parent = 0) {
	...
	if ( $file )
	{
		if($post_parent != 0)
		{
			$file2 = strstr($file, "wp-content/");
			$query = "update wp_posts set attachment_url = '{$file2}' where ID = {$post_parent}";
	        ...
	        $result = $wpdb->get_results($query);
       		if($result != null)
       		{
        		$term_taxonomy_id = $result[0]->term_taxonomy_id;
	                ...
	                $query2 = "select post_content, post_title, post_date, id, attachment_url from wp_posts  where id in ({$query}) order by post_date desc ";
	//		echo $query2;
			$result = $wpdb->get_results($query2);
